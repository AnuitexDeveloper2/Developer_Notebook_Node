...
export const get = async (topicId: string): Promise<CommonResponse<Array<appointmentModel>>> => {
    let result: CommonResponse<Array<appointmentModel>> = { data: null };
    const resultData = await repo.get(topicId)
    if (!resultData) {
        result.error = MyErrors.SOMETHING_WENT_WRONG
    }
    result.data = resultData;
